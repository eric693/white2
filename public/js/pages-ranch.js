...
App.page('ranch', {
  help: {
    "intro": "養動物定期產出（蛋、奶…），收成進背包賣錢；別人可以來偷。",
    "steps": [
      "「動物」設售價、產出間隔與每日產量，並指定產出哪個物品。",
      "看門動物（設了看門％）不生產，改成降低被偷機率。",
      "「設定」可調偷竊失敗的罰金金額。"
    ],
    "notes": [
      "未領取的產物有上限（每日產量 × 可囤天數），滿了就停止累積，要玩家自己來收。",
      "設施等級的『產量 +N%』會同時提高每次產出與囤積上限。",
      "偷竊失敗罰金自 2026-09 起是固定金額（預設 1,000），魚缸偷魚讀的是同一個值，兩邊一致。以前是隨機抽，同一次失敗可能罰 1 塊也可能罰 5,000，玩家完全無法預期。",
      "罰金是全額扣、不看玩家有沒有錢，可以把餘額扣成負數（欠款）。"
    ]
  },
