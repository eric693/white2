...
App.page('gather', {
  help: {
    "intro": "整個冒險經濟的地基：釣魚／挖礦／伐木／採集／狩獵的冷卻、體力、掉落物與工具，都在這裡調。",
    "steps": [
      "先在「設定」決定每日體力、各類冷卻秒數，以及是否「禁止徒手」（沒工具就不能採集）。",
      "「掉落物」加物品時，權重越大越常抽到；稀有度只影響顯示與任務條件，實際機率看權重。",
      "「道具」是工具：幸運％提高稀有掉落、冷卻縮短％加快速度；設了耐久就會用久了壞掉，要花錢修。",
      "工具壞掉＝直接沒工具，不會自動退回低階那把（2026-09 改）。玩家要先 /修理 或買新的才能繼續採集，訊息會直接告訴他壞的是哪一把、修多少錢。",
      "「製作手續費 %」按材料市值抽成、每做一次收一次，直接銷毀回收星幣；「額外失敗率 %」直接扣在每個配方自己的成功率上（配方 90% ＋ 這裡 10 ＝ 實際 80%），失敗一樣扣材料與費用。兩個都填 0 就跟以前一樣。",
      "改完不必重開機器人，玩家下一次指令就吃到新設定。"
    ],
    "notes": [
      "物品的『價格』是 NPC 收購價，會被「世界動態」的物價調整影響。",
      "刪掉掉落物會連玩家背包裡的紀錄一起清掉，圖鑑也會少一格，請先停用觀察再刪。"
    ],
    "terms": [
      [
        "體力",
        "每天午夜（台北時間）回滿，釣魚挖礦共用同一池，可在特殊商店買。"
      ],
      [
        "權重",
        "抽獎的相對比例，不是百分比。某物權重 250、全部加起來 1000，就是 25%。"
      ]
    ]
  },
    ...
    el.innerHTML = `
      <div class="card" style="max-width:720px" id="cfgwrap">
        ...
        <div class="field">${H.toggle('require_tool', c.require_tool ?? 1, '禁止徒手採集（沒有可用工具就不能釣魚／挖礦／伐木／採集／狩獵）')}
          <div class="hint">工具壞掉、還沒買、或<b>被舊的物資貸款抵押著</b>（該制度已停辦，僅剩未還清的舊借據）時都算沒工具，要先修理／購買／贖回。關掉＝像以前一樣可以徒手（沒加成）。</div></div>
        <div class="form-row">
