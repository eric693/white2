...
App.page('wheels', {
  help: {
    "intro": "角色轉盤：玩家抽一位角色，可收藏、可看抽取紀錄，支援標籤篩選與權重。",
    "steps": [
      "先建立轉盤，設每日抽取次數與是否公開結果。",
      "加入角色，填名字、介紹、圖片與連結。",
      "權重越大越常被抽到；標籤讓玩家可以只抽特定類型。"
    ],
    "notes": [
      "開啟「小卡模式」會把角色做成一張圖片，比純文字好看很多。",
      "開啟「不重複」時，玩家會先抽完所有角色才重新開始新一輪。"
    ]
  },
...
App.page('reminders', {
  help: {
    "intro": "定時提醒：單次、每日、每週或每月，可指定對象與頻道。",
    "steps": [
      "寫提醒內容，選頻道與要標記的對象。",
      "選頻率與時間。"
    ],
...
App.page('users', {
  help: {
    "intro": "後台帳號與權限：決定誰能登入後台、能看到哪些模組、能管哪些伺服器。",
    "steps": [
      "新增帳號並設定密碼；開給客戶的帳號，角色一定選「管理員（限定功能與伺服器）」。",
      "勾選這個帳號可以使用的功能模組（設定區建議整區不勾，需要時只給媒體庫與功能權限設定）。",
      "「可管理的伺服器」一定要勾他那一台——不勾等於空綁定，他什麼都看不到。"
    ],
    "notes": [
      "總管理員能看到全部功能與所有伺服器，請謹慎給予。",
      "編輯客戶帳號請用總管理員登入：一般管理員在這頁只列得出自己綁定的伺服器，勾不到別台。",
      "伺服器管理會看到所有客戶的伺服器清單與邀請連結；帳號權限能自己升級成總管理員，兩者都別給客戶。",
      "只勾檢視類模組的帳號進到頁面會是唯讀，操作按鈕會被鎖住。"
    ]
  },
