...
App.page('home', {
  help: {
    ...
    "steps": [
      ...
      "「同居能力」決定每位角色搬進玩家家裡會帶什麼能力。一區只有一個能力（🌾農地照顧＝收成＋播種、🐠魚缸照顧＝餵食＋領星幣…），不再拆成「收成」和「種植」兩個。",
      "同居角色可以指派一個「工作區域」（農地／溫室／牧場／孵化室／魚缸），一位角色就把那一區包辦到底。"
    ],
    "notes": [
      "玩家可以在寵物面板「👋 送走一隻寵物」把養不起的寵物送走（回收購入價的一半，養成資料消失，下一期寵物稅就會少算一隻）。以前只能領養不能送走，等於養了就永遠得繳稅。",
      ...
      "工作區域跟角色的能力一一對應：🌾農地＝農地照顧、🏡溫室＝溫室照顧、🐄牧場＝牧場照顧（或擊退小偷）、🥚孵化室＝孵化室照顧、🐠魚缸＝魚缸照顧、🍳廚房＝廚房做飯。挖礦、釣魚、賣價加成那類能力每天自動生效，不能（也不需要）指派區域。要讓某位角色能顧某一區，就在這頁把他的能力改成該區的能力。",
