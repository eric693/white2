...
App.page('tickets', {
  help: {
    "intro": "客服單：玩家按按鈕就開一個只有他與客服看得到的頻道。",
    "steps": [
      "建立面板，設標題、說明與要發到哪個頻道。",
      "指定客服身分組與客服單要開在哪個分類。",
      "按「發布面板」把按鈕發到頻道上。"
    ...
    "notes": [
      "玩家按下按鈕會直接開單進入頻道，不需要先填表單。",
      "結案只有客服能按：開單者看得到「關閉客服單」按鈕，但按下去會被擋，避免玩家自己把還沒處理完的單關掉。開單者可以持續在頻道裡回覆補充。",
      "所以「客服身分組」一定要設，沒設就沒有人能結案。",
      "結案後頻道會封存，紀錄仍可在後台查詢。",
      "頻道被手動刪掉的殘留單，系統會自動標成結案（結案人記為系統）。"
    ]
  },
...
App.page('levels', {
  help: {
    "intro": "聊天累積經驗值，升到指定等級自動給身分組。",
    "steps": [
      "設定每則訊息的經驗值與防洗版冷卻。",
      "設定各等級要給的身分組。"
    ],
    "notes": [
      "冷卻期間內的訊息不計經驗值，避免洗頻刷等。"
    ]
  },
