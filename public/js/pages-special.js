...
App.page('special', {
  help: {
    "intro": "玩家花星幣兌換虛擬獎勵（身分組、稱號、客製服務）。可自動發放的當場入袋，其餘進管理員待辦。",
    "steps": [
      "新增商品，設價格、庫存與每人每期上限。",
      "要自動發放就設「發放物品」或「發放體力」，玩家換完立刻入袋、不用你處理。",
      "其餘商品兌換後會通知管理員，處理完在後台標記為已處理。"
    ],
    "notes": [
      "開啟「越換越貴」時，同一人每換一次價格就翻倍，避免有錢人整批掃貨。",
      "有未還貸款或欠稅的玩家不能兌換，先把債還了才行。",
      "嫌私訊太吵：把「兌換通知方式」改成「只發到預設通知頻道」並選一個管理員頻道，或選「完全不通知」——兌換紀錄一律會進這頁的待辦，不會因為關通知而漏單。",
      "商店面板一定要由「璃白Yu光管家」發出去：面板上的下拉選單，Discord 只會把互動送給發訊息的那隻機器人。改了商品或價格後按「發布面板」即可，系統會自動交給管家處理（管家沒上線就排隊，上線後補做）。",
      "2026-09 拆成兩隻機器人之前發的舊面板已經失效，玩家點了只會看到「這個商店面板是舊版的」；按一次「發布面板」，系統會刪掉舊的並由管家重發一則。"
    ],
    "terms": [
      [
        "重置週期",
        "每人上限與累進價格重新計算的時間點，可設每月或每兩週的週一。"
      ]
    ]
  },
      ...
      <div class="card" style="max-width:760px" id="cfgwrap">
        ...
        <div class="field"><label>兌換通知要發到哪</label>
          <select name="notify_mode">
            ...
            <option value="none" ${c.notify_mode==='none'?'selected':''}>完全不通知（兌換單照樣進下方待辦，自己來後台看）</option>
          </select>
