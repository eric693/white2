...
// ---- 買 ----
// 手續費：買進與賣出各收 fee_pct%（2026-09 起統一 1.5%／1.5%），
// 除此之外不再收任何股票稅費——證券稅（依持股市值課）已經一併移除。
// 當沖開放：當天買進當天就能賣，沒有最低持有時間。
(function migrateStockFee() {
  const { getSetting, setSetting } = require('../../db');
  if (getSetting('stock_fee_2026_09', '0') === '1') return;
  try {
    // 只調還停在舊預設 2% 的伺服器，後台自己改過的值不動
    db.prepare('UPDATE market_config SET fee_pct = 1.5 WHERE fee_pct = 2').run();
    setSetting('stock_fee_2026_09', '1');
  } catch (e) { console.error('股票手續費預設值搬遷失敗：', e.message); }
})();

// 同居能力「股市手續費減免」：把手續費率往下調（不會低於 0）。
...
function buy(gid, uid, username, key, shares) {
  ...
  db.transaction(() => {
    ...
    logCoins(gid, uid, -(cost + fee), '買股票', `${s.name} ×${shares}${fee ? `（含手續費 ${fee}）` : ''}`);
    db.prepare(
...
function marketEmbed(gid) {
  ...
  return new EmbedBuilder().setColor(brandColor())
    ...
    .setFooter({ text: `下次結算 ${new Date(next).toLocaleTimeString('zh-TW', { timeZone: 'Asia/Taipei', hour: '2-digit', minute: '2-digit' })}　買賣手續費各 ${c.fee_pct}%　漲跌停 ±${c.limit_pct}%` });
}
