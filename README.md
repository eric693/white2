...
| `affinity` | 角色：逛街**隨機事件制**（遇到角色的機率已大幅降低）、送禮（喜好倍率、角色多時自動翻頁）、同居與同居能力。同居與寵物**沒有數量上限**，轉盤名單與同居名單完全分離；同居角色可指派「工作區域」，一位角色即可包辦該區完整自動化；**收成／照顧類每 5 分鐘不間斷執行**（成熟就收、收完補種），只有每日配給類是每天 8:40 一次 |
