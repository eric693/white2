...
1. 角色一定選 **`staff`（管理員）**；選 `admin` 就是全功能全伺服器，什麼都藏不住。
2. **「可管理的伺服器」一定要勾他那一台**。不勾＝空綁定，該帳號什麼都看不到
   （`allowedGuildsFor` 是 fail closed；以前會退回主伺服器，等於把作者自己的伺服器露給客戶）。
3. **一定要用 `admin` 帳號去編輯客戶帳號**。`staff` 在帳號頁只列得出自己綁定的伺服器，
   用它編輯別人會勾不到對方那台（後端 `sanitizeGuilds` 會保留看不見的綁定，但畫面上仍然勾不到）。
4. 「設定」區預設**整區不要勾**，需要時只單獨給 `media`（媒體庫）與 `perms`（功能權限設定）：
   - `blacklist`／`appearance` — 作者專用，非 admin 在畫面上根本看不到這兩個勾，
     後端 `sanitizePerms` 也會濾掉（`OWNER_ONLY`）
   - `guilds`（伺服器管理）— 會看到**所有**客戶的伺服器清單與邀請連結
   - `users`（帳號權限）— 能自己升級成 admin
   - `system`（系統狀態）— 錯誤紀錄會一併包含「全站層級」（`guild_id = ''`）的內容
...
內建方案：秘書 `free` / `trial` / `standard` / `pro`，管家 `free` / `trial` / `basic` / `advanced`。
兩隻**同一套級距、同一組價格**（`basic`／`advanced` 的顯示名稱也叫「標準版」「專業版」）：

| 方案 | 價格 | 內容 |
| --- | --- | --- |
| 免費版 | 免費 | 秘：歡迎／投票／關鍵字／公告；管：採集／家園／圖鑑／面板／說明 |
| 試用版 | 免費 | 全功能 14 天 |
| 標準版 | NT$99／月・NT$990／年 | 秘：＋生日／表情身分組／抽獎／轉盤／等級／提醒；管：＋農地／牧場／廚房／家具／交易 |
| 專業版 | NT$199／月・NT$1,990／年 | 全功能 |

⚠️ **`blacklist`（黑名單）與 `appearance`（外觀自訂）不在任何方案裡**，也不是付費牆管的
——它們是「後台模組」層級的作者專用鑰匙，見下方「後台操作備忘」。
...
功能介紹頁（`/features#apply`）有一張公開申請表，走 `POST /api/apply`（不需登入，
每個 IP 每小時 5 次）。送出的單子進 `guild_applications`，在後台「伺服器管理」最上面那張
「線上申請」表核准／婉拒；**核准且填了伺服器 ID 就會自動加進白名單**，對方直接重邀即可。

### 邀請紀錄

每一次 `guildCreate`（含被白名單擋下的那次）都會 `recordInvite()`，累加 `guilds` 的
`invite_count`／`last_invite_at`／`invited_roles`。後台「伺服器管理」上方顯示：
邀請過幾台／已開通／待核准／在線／累計邀請次數／本月有邀請。
`invited_roles` 只認得**當前行程扮演的那一隻**，所以要兩隻都跑著才分得出來。

### ⚠️ 換 Discord 應用程式的代價

面板按鈕的互動**只會送到「發出那則訊息的應用程式」**。換了 app（例如秘書 2026-09 換成
`1546031775606775818`）之後，舊訊息的按鈕全部變成死的——畫面上看得到、點下去沒反應。
所有常駐面板都要重新發布一次：生日驗證、音樂控制、客服單各面板、表情身分組、抽獎、公告按鈕。
**已開單的客服單頻道內的「關閉客服單」救不回來**（一次性訊息），只能用 `/關單` 或刪頻道。
